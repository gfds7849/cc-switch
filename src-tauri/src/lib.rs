...
pub fn run() {
    ...
    builder = builder.setup(|app| {
            ...
            #[cfg(any(target_os = "linux", all(debug_assertions, windows)))]
            {
                ...
                {
                    if portable::is_portable() {
                        log::info!("Skipping Windows debug deep-link registration in portable mode");
    ...
    if cfg!(target_os = "windows") && portable::is_portable() {
        let main_window = context.config_mut().app.windows.iter_mut().find(|window| window.label == "main").unwrap_or_else(|| panic!("main window configuration not found"));
...
pub fn save_window_state_before_exit(app_handle: &tauri::AppHandle) {
    if portable::is_portable() {
        if portable_window_state::save(app_handle).is_err() {
            log::error!("portable window state save failed");
        }
        return;
    }
